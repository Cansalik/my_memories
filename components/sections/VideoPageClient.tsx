...
export default function VideoPageClient({ videos }: { videos: GalleryVideo[] }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12 sm:py-16 md:py-24">
      <motion.div className="mb-10 sm:mb-14" initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}>
        <Link href="/" className="inline-flex items-center gap-2 mb-6 sm:mb-8 group"
          style={{ color:"rgba(255,247,230,.35)", fontFamily:"'Lato',sans-serif", fontSize:"0.75rem", letterSpacing:"0.15em" }}>
        ...
        <div className="text-center">
          ...
          <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(2rem,7vw,4rem)", color:"#FFF7E6", textShadow:"0 0 40px rgba(245,210,122,.2)" }}>Tüm Videolar</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {videos.map((video, i) => (
          <motion.button key={video.id} className="group relative w-full rounded-2xl overflow-hidden text-left"
            ...
            transition={{ duration:0.5, delay:(i%3)*.08 }}
            whileHover={{ scale:1.02 }} onClick={() => setActive(i)}>
            <div className="absolute inset-0">
              {video.thumbnail_url ? <img src={video.thumbnail_url} alt={video.title} className="w-full h-full object-cover" /> : <FilmPlaceholder index={i} />}
            </div>
            ...
            {video.featured && (
              <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-0.5 rounded-full"
                style={{ background:"rgba(11,29,58,.88)", border:"1px solid rgba(245,210,122,.3)", backdropFilter:"blur(6px)" }}>
                <span style={{ color:"#F5D27A", fontSize:"0.55rem" }}>✦</span>
                <span style={{ color:"rgba(245,210,122,.7)", fontFamily:"'Lato',sans-serif", fontSize:"0.55rem" }}>Öne Çıkan</span>
              </div>
            )}
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="flex items-center justify-center rounded-full opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all duration-300"
                style={{ width:48, height:48, background:"rgba(11,29,58,.85)", border:"1.5px solid rgba(245,210,122,.5)", backdropFilter:"blur(8px)" }}>
            ...
            {video.duration && (
              <div className="absolute bottom-3 right-3 px-2 py-0.5 rounded"
                style={{ fontFamily:"'Lato',sans-serif", fontSize:"0.6rem", background:"rgba(7,15,30,.88)", border:"1px solid rgba(245,210,122,.2)", color:"rgba(245,210,122,.8)" }}>
                {video.duration}
              </div>
            )}
            <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 pr-12 sm:pr-14">
              <p style={{ fontFamily:"'Playfair Display',serif", color:"#FFF7E6", fontWeight:500, fontSize:"clamp(0.8rem,2.5vw,0.9rem)" }}>{video.title}</p>
      ...
      <AnimatePresence>
        {active !== null && (
          <VideoModal
            ...
            onPrev={() => setActive(i => i===null?null:(i-1+videos.length)%videos.length)}
            onNext={() => setActive(i => i===null?null:(i+1)%videos.length)}
          />
        )}
      </AnimatePresence>
    </div>
