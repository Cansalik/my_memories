...
export default function GalleryPageClient({ photos }: { photos: GalleryPhoto[] }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12 sm:py-16 md:py-24">
      <motion.div className="mb-10 sm:mb-14" initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.7 }}>
        ...
        <div className="text-center">
          ...
          <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(2rem,7vw,4rem)", color:"#FFF7E6", textShadow:"0 0 40px rgba(245,210,122,.2)" }}>Tüm Fotoğraflar</h1>
      ...
      {/* Masonry grid — mobilde 2, sm'de 3, lg'de 4 kolon */}
      <div className="columns-2 sm:columns-3 lg:columns-4 gap-3">
        {photos.map((photo, i) => (
          <motion.div key={photo.id}
            className="break-inside-avoid mb-3 group relative rounded-xl overflow-hidden cursor-pointer"
            style={{ aspectRatio:"1/1", border:photo.featured?"1px solid rgba(245,210,122,.25)":"1px solid rgba(245,210,122,.1)", boxShadow:"0 4px 20px rgba(0,0,0,.35)" }}
            initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}
            transition={{ duration:0.5, delay:(i%4)*.06 }}
            ...
            {/* Hover overlay — mobilde her zaman göster */}
            <div className="absolute inset-0 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-300 flex items-end p-2 sm:p-3"
              style={{ background:"linear-gradient(to top,rgba(11,29,58,.9) 0%,rgba(11,29,58,.3) 55%,transparent 100%)" }}>
              <div>
                <p style={{ fontFamily:"'Playfair Display',serif", color:"#FFF7E6", fontSize:"clamp(0.7rem,2.5vw,0.9rem)", fontWeight:500 }}>{photo.caption}</p>
                <p style={{ fontFamily:"'Lato',sans-serif", color:"rgba(245,210,122,.65)", fontSize:"0.6rem" }}>{photo.date}</p>
              </div>
            </div>
          </motion.div>
        ))}
