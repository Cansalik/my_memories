...
function FilmPlaceholder({ index }: { index:number }) {
  ...
  return (
    <div className="w-full h-full relative" style={{ background:"linear-gradient(135deg,rgba(17,35,71,.98),rgba(22,42,80,.9))" }}>
      ...
      <div className="absolute inset-0 flex items-center justify-center"><span style={{ color:"rgba(245,210,122,.18)", fontSize:"1.6rem" }}>▶</span></div>
    </div>
  );
}
  ...
  return (
    <section className="relative py-12 sm:py-16 px-4">
      ...
      <motion.div className="text-center mb-5"
        ...
        <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(2rem,6vw,3.5rem)", color:"#FFF7E6", textShadow:"0 0 30px rgba(245,210,122,.18)" }}>Video Galerimiz</h2>
      </motion.div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between max-w-5xl mx-auto mb-6 sm:mb-8 px-1 gap-3">
        <div className="flex items-center gap-2">
          <div className="w-1 h-1 rounded-full" style={{ background:"rgba(232,164,184,.5)" }} />
          <span style={{ fontFamily:"'Lato',sans-serif", fontSize:"0.7rem", color:"rgba(255,247,230,.3)" }}>Öne çıkan videolar</span>
        </div>
        <Link href="/videolar" className="inline-flex items-center gap-2 px-4 py-2 rounded-full self-start sm:self-auto"
          style={{ background:"rgba(245,210,122,.06)", border:"1px solid rgba(245,210,122,.18)" }}>
          <span style={{ fontFamily:"'Lato',sans-serif", fontSize:"0.7rem", color:"rgba(245,210,122,.7)" }}>Tüm Videolar</span>
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
        {featured.map((video, i) => (
          <motion.button key={video.id} className="group relative w-full rounded-2xl overflow-hidden text-left"
            style={{ aspectRatio:"16/9", display:"block", border:video.featured?"1px solid rgba(245,210,122,.25)":"1px solid rgba(245,210,122,.1)", boxShadow:"0 4px 24px rgba(0,0,0,.4)" }}
            initial={{ opacity:0, y:18 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:0.5, delay:i*0.08 }}
            ...
            <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl pointer-events-none" style={{ boxShadow:"inset 0 0 0 1.5px rgba(245,210,122,.4),inset 0 0 32px rgba(245,210,122,.06)" }} />
            <div className="absolute inset-0 flex items-center justify-center">
              <motion.div className="flex items-center justify-center rounded-full"
                style={{ width:52, height:52, background:"rgba(11,29,58,.85)", border:"1.5px solid rgba(245,210,122,.55)", backdropFilter:"blur(8px)", boxShadow:"0 0 22px rgba(245,210,122,.22)" }}
                whileHover={{ scale:1.14 }}>
                <motion.div className="absolute inset-0 rounded-full" style={{ border:"1px solid rgba(245,210,122,.25)" }}
                  animate={{ scale:[1,1.65,1], opacity:[0.5,0,0.5] }} transition={{ duration:2.5, repeat:Infinity, delay:i*.3 }} />
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 pr-12 sm:pr-14">
              <p style={{ fontFamily:"'Playfair Display',serif", color:"#FFF7E6", fontWeight:500, fontSize:"clamp(0.82rem,2.5vw,0.95rem)", lineHeight:1.3 }}>{video.title}</p>
      ...
      <motion.div className="flex justify-center mt-6 sm:mt-8"
        ...
        <Link href="/videolar" className="group relative inline-flex items-center gap-3 px-6 sm:px-7 py-3 rounded-full overflow-hidden"
