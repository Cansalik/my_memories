...
export default function VideoModal({ video, onClose, onPrev, onNext }: Props) {
  ...
  return (
    <AnimatePresence>
      {video && (
        /* 1. DEĞİŞİKLİK: items-center kaldırıldı, overflow-y-auto eklendi ve padding artırıldı */
        <motion.div className="fixed inset-0 z-50 flex justify-center overflow-y-auto p-4 sm:p-8 md:p-12"
          style={{ background:"rgba(3,6,14,.96)", backdropFilter:"blur(18px)" }}
          initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} onClick={onClose}>
          
          {/* 2. DEĞİŞİKLİK: mb-[50px] (50px alt boşluk) ve h-fit (içerik kadar yükseklik) eklendi */}
          <motion.div className="relative w-full mb-[50px] h-fit" style={{ maxWidth:900 }}
            ...
            <div className="rounded-xl sm:rounded-2xl overflow-hidden"
              ...
              <div className="px-4 sm:px-5 py-5 sm:py-6 flex items-center justify-between"
                style={{ borderTop:"1px solid rgba(245,210,122,.08)" }}>
                <div>
                  <p style={{ fontFamily:"'Playfair Display',serif", color:"#FFF7E6", fontWeight:500, fontSize:"clamp(0.9rem,2.5vw,1.1rem)" }}>{video.title}</p>
                  <p style={{ fontFamily:"'Lato',sans-serif", color:"rgba(245,210,122,.55)", fontSize:"0.78rem", marginTop:4 }}>{video.date}</p>
                </div>
