    ...
    <AnimatePresence>
      {photo && (
        <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 md:p-8"
          ...
          <motion.div className="relative w-full" style={{ maxWidth:780 }}
            ...
            <div className="rounded-xl sm:rounded-2xl overflow-hidden"
              ...
              <div style={{ position:"relative", aspectRatio:"4/3", background:"linear-gradient(135deg,rgba(17,35,71,.95),rgba(30,50,90,.8))" }}>
                ...
                {/* Prev/Next on image — larger touch targets for mobile */}
              ...
              <div className="px-4 sm:px-5 py-3 sm:py-4" style={{ background:"rgba(11,29,58,.98)", borderTop:"1px solid rgba(245,210,122,.08)" }}>
                <p style={{ fontFamily:"'Playfair Display',serif", color:"#FFF7E6", fontWeight:500, fontSize:"clamp(0.9rem,2.5vw,1.1rem)" }}>{photo.caption}</p>
                <p style={{ fontFamily:"'Lato',sans-serif", color:"rgba(245,210,122,.55)", fontSize:"0.78rem", marginTop:4 }}>{photo.date}</p>
              </div>
            </div>
          </motion.div>
        </motion.div>
