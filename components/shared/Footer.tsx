...
export default function Footer() {
  ...
  useEffect(() => {
    fetch("/api/song")
      .then(r => r.json())
      .then(d => { if (d.title) setSongTitle(d.title); if (d.youtube_id) setYoutubeId(d.youtube_id); })
      .catch(() => {});
  }, []);
  ...
  const togglePlay = () => { if (!ready || !playerRef.current) return; playing ? playerRef.current.pauseVideo() : playerRef.current.playVideo(); };
  ...
  return (
    <footer
      ...
      style={{ height: 70, background: "linear-gradient(90deg,rgba(7,15,30,.97),rgba(11,29,58,.97),rgba(7,15,30,.97))", borderTop: "1px solid rgba(245,210,122,.12)", backdropFilter: "blur(16px)", boxShadow: "0 -4px 32px rgba(0,0,0,.4)" }}
    >
      {/* Hidden YT iframe */}
      ...
      <div className="flex items-center gap-2.5 pl-5 flex-shrink-0" style={{ minWidth: 170 }}>
        <motion.span className="text-gold text-base" animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 3, repeat: Infinity }} style={{ textShadow: "0 0 10px rgba(245,210,122,.6)" }}>✦</motion.span>
        <div>
      ...
      <div className="flex-1 flex flex-col items-center gap-1 px-4" style={{ maxWidth: 460, margin: "0 auto" }}>
        <div className="flex items-center gap-3 w-full justify-center">
          ...
          {/* Play button */}
          <motion.button onClick={togglePlay} className="flex-shrink-0 flex items-center justify-center rounded-full" style={{ width: 32, height: 32, background: ready ? "linear-gradient(135deg,#F5D27A,#C9A84C)" : "rgba(245,210,122,.15)", border: "none", cursor: ready ? "pointer" : "default", boxShadow: ready ? "0 0 12px rgba(245,210,122,.3)" : "none", transition: "background 0.4s" }} whileHover={ready ? { scale: 1.1 } : {}} whileTap={ready ? { scale: 0.92 } : {}}>
        ...
        <div className="flex items-center gap-2 w-full" style={{ maxWidth: 360 }}>
          <span style={{ fontFamily: "'Lato',sans-serif", fontSize: "0.56rem", color: "rgba(255,247,230,.25)", minWidth: 24, textAlign: "right" }}>{fmt(currentTime)}</span>
          ...
          <span style={{ fontFamily: "'Lato',sans-serif", fontSize: "0.56rem", color: "rgba(255,247,230,.25)", minWidth: 24 }}>{fmt(duration)}</span>
        </div>
      </div>

      {/* Right — copyright */}
      <div className="flex items-center justify-end pr-5 flex-shrink-0" style={{ minWidth: 170 }}>
        <p style={{ fontFamily: "'Lato',sans-serif", fontSize: "0.58rem", color: "rgba(255,247,230,.14)", letterSpacing: "0.08em", textAlign: "right" }}>© {new Date().getFullYear()} · Tüm hakları saklıdır</p>
      </div>
    </footer>
  );
}
